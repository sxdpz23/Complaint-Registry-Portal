...
return TRUE;
	...
	public function rules()
	{
		...
		return array(
			array(' location, complaint','required'),
			array('name, workAssignTo, status, updatedBy', 'length', 'max'=>30),
			array('location', 'length', 'max'=>50),
			array('contact', 'length', 'max'=>20),
			array('complaint, actionTaken, feedback', 'length', 'max'=>1000),
			array('workDoneOn', 'safe'),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, name, date, location, contact, complaint, workAssignTo, actionTaken, status, workDoneOn, updatedBy, feedback', 'safe', 'on'=>'search'),
		);
	}
